...
class Utils {
	...
	public static function make_quantity_input_interactive( $quantity_html, $wrapper_attributes = '', $child_product_id = null ) {
		...
		if (
			...
			$processor->set_attribute( 'data-wp-on--input', 'wooocommerce/add-to-cart-with-options::actions.handleQuantityInput' );
			$processor->set_attribute( 'data-wp-on--change', 'wooocommerce/add-to-cart-with-options::actions.handleQuantityChange' );
		}

...
class WC_Post_Data {
	 ...
	 * @param bool       $force   Whether the attributes were forcibly updated.
	 *
	 * @return void
	 *
	 * @since 10.2.0
	...
	public static function on_product_attributes_updated( $product, $force = false ) {
		if ( $product->is_type( 'variable' ) ) {
			if ( ! $force ) {
				self::delete_empty_stale_variation_attribute_meta( $product );
			}
	...
	/**
	 * Deletes empty child variation attribute meta that no longer maps to a parent variation attribute.
	 *
	 * Non-empty orphaned meta is preserved because an incomplete parent update can temporarily make valid
	 * variation assignments appear stale. Keeping those values makes the update recoverable.
	 *
	 * @param WC_Product $product The variable product whose attributes were updated.
	 *
	 * @return void
	 */
	private static function delete_empty_stale_variation_attribute_meta( $product ) {
		global $wpdb;
		...
		foreach ( $stale_rows as $row ) {
			...
			if ( delete_metadata_by_mid( 'post', (int) $row->meta_id ) ) {
